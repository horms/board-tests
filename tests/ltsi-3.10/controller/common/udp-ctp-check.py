...
class Test:
    def __init__(self, board_hostname, board_username, log_file, types, device):
...
# setting command with given arguments
    def set_cmd_args(self, option):
        return [ 'iperf', option, '>', self.log_file ]
...
#Preparing a command before called
    ...
    def udp_tcp_check(self, info_str, option):
        retcode = True
        if self.device == 'BOARD':
                cmd = self.board_cmd_args(self.prepare_cmd(self.set_cmd_args(option)))
	elif self.device == 'HOSTPC':
		cmd = self.set_cmd_args(option)
	else:
		print "Unknown device! Please tell me the device: BOARD or HOSTPC"
		return False
...
# Running:
    def run(self):
        ...
        if self.types == 'UDP':
                ...
                print "Unknown type! Please tell me type UDP or TCP"
                return False
        val = self.udp_tcp_check(info_str, option)
        if not val:
...
def usage():
        fatal_err(
"Usage: udp-tcp-check.py [options] LOCAL_TTY \\\\n" +
...
"    UDP_FILE:        Is file to write udp-log from board"
"\n" +
"  options:\n" +
...
"\n" +
"  e.g:\n" +
"    udp-tcp-check.py armadillo800 root /tmp/udp-check UDP BOARD \n" +
""
    )
